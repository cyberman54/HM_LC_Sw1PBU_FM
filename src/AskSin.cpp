...
{ // initialize CC1101
	...
	while (readReg(CC1101_MARcurStatTE, CC1101_STATUS) != 1)
...
boolean CC::sendData(uint8_t *buf, uint8_t burst)
...
{															  // read data packet from RX FIFO
	...
	if (buf[0] > 0)
		Serial << pHexL(&buf[1], buf[0]) << pTime();
#endif
...
uint8_t CC::sendSPI(uint8_t val)
{				// send byte via SPI
...
{									// write multiple registers into the CC1101 IC via SPI
	...
	for (uint8_t i = 0; i < len; i++)
		sendSPI(buf[i]); // send value
	cc1101_Deselect();   // deselect CC1101
}
void CC::readBurst(uint8_t *buf, uint8_t regAddr, uint8_t len)
{								   // read burst data from CC1101 via SPI
	cc1101_Select();			   // select CC1101
	wait_Miso();				   // wait until MISO goes low
	sendSPI(regAddr | READ_BURST); // send register address
	for (uint8_t i = 0; i < len; i++)
		buf[i] = sendSPI(0x00); // read result byte by byte
	cc1101_Deselect();			// deselect CC1101
}
uint8_t CC::readReg(uint8_t regAddr, uint8_t regType)
...
void CC::writeReg(uint8_t regAddr, uint8_t val)
...
void LD::poll()
{
	if ((nTime == 0) || (nTime > millis()))
		return; // nothing to do or wrong time to do
...
{ // task scheduler
	if (recv.data[0] > 0)
		recv_poll(); // trace the received string and decide what to do further
	if (send.counter > 0)
		send_poll(); // something to send in the buffer?
	if (conf.act > 0)
		send_conf_poll(); // some config to be send out
	if (pevt.act > 0)
		send_peer_poll(); // send peer events
	power_poll();
	ld.poll();
}
void HM::send_out()
{
	if (bitRead(send.data[2], 5))
		send.retries = maxRetries; // check for ACK request and set max retries counter
	else
		send.retries = 1; // otherwise send only one time

	send.burst = bitRead(send.data[2], 4); // burst necessary?

	if (memcmp(&send.data[7], HMID, 3) == 0)
...
{
	if (memcmp(regDev.pairCentral, broadCast, 3) == 0)
...
void HM::sendPeerREMOTE(uint8_t button, uint8_t longPress, uint8_t lowBat)
{
	...
	//	   COUNTER  => "02,2", } },
	if (button > maxChannel)
		return; // channel out of range, do nothing

	if (doesListExist(button, 4) == 0)
	{ // check if a list4 exist, otherwise leave
...
void HM::sendPeerRAW(uint8_t cnl, uint8_t type, uint8_t *data, uint8_t len)
{
	...
	if (cnl > maxChannel)
		return; // channel out of range, do nothing
	if (pevt.act)
		return; // already sending an event, leave
	if (doesListExist(cnl, 4) == 0)
	{ // check if a list4 exist, otherwise leave
		//Serial << "sendPeerREMOTE failed\n";
		return;
	}
...
void HM::printConfig()
{
	...
	{ // count through the channel
		for (uint8_t j = 0; j < 20; j++)
		{ // count through the lists
			if (!getSliceDetail(i, j, &peer[0], &sV))
				continue; // get the slice details
	...
	{ // step through all channels
		...
		uint8_t lst = 0;
		if (doesListExist(i + 1, 3))
			lst = 3; // check for list3
		if (doesListExist(i + 1, 4))
			lst = 4; // check for list4
		if (lst == 0)
			continue; // no list3 or list4 therefore next i

		// step through the peers
		for (uint8_t j = 0; j < 20; j++)
		{ // step through all possible peers
			if (!getPeerByIdx(i + 1, j, &peer[4]))
				continue; // ask for the peer by channel and peer index
			peer[3] = j;  // store the peer channel byte
			if (!getSliceDetail(i + 1, lst, &peer[0], &sV))
				continue;																																  // ask for the slice details as list3
...
void HM::isrGDO0event(void)
{
	...
	if (hm.cc.receiveData(hm.recv.data))
	{							 // is something in the receive string
...
void HM::recv_poll(void)
{ // handles the receive objects
	...
	if (memcmp(&recv.data[7], HMID, 3) == 0)
		...
		recv.forUs = 0;
	if (memcmp(&recv.data[7], broadCast, 3) == 0)
...
// show debug message
	...
	if ((isPairKnown(recv_reID) == 0) && (isPeerKnown(recv_reID) == 0))
	{				// check against peers
#if defined(AS_DBG) // some debug message
//Serial << "pair/peer did not fit, exit\n";
	...
	if ((recv.forUs) && (recv_isMsg) && (recv_msgTp == 0x01))
	{ // message is a config message
		if (recv_by11 == 0x01)
			recv_ConfigPeerAdd(); // 01, 01
		else if (recv_by11 == 0x02)
			recv_ConfigPeerRemove(); // 01, 02
		else if (recv_by11 == 0x03)
			recv_ConfigPeerListReq(); // 01, 03
		else if (recv_by11 == 0x04)
			recv_ConfigParamReq(); // 01, 04
		else if (recv_by11 == 0x05)
			recv_ConfigStart(); // 01, 05
		else if (recv_by11 == 0x06)
			recv_ConfigEnd(); // 01, 06
		else if (recv_by11 == 0x08)
			recv_ConfigWriteIndex(); // 01, 08
		else if (recv_by11 == 0x09)
			recv_ConfigSerialReq(); // 01, 09
		else if (recv_by11 == 0x0A)
			recv_Pair_Serial(); // 01, 0A
		else if (recv_by11 == 0x0E)
			recv_ConfigStatusReq(); // 01, 0E
#if defined(AS_DBG)					// some debug message
		else
	...
	if ((recv.forUs) && (recv_isMsg) && (recv_msgTp == 0x02))
	{ // message seems to be an ACK
		send.counter = 0;
	}
...
void HM::send_poll(void)
{ // handles the send queue
	if ((send.counter <= send.retries) && (send.timer <= millis()))
	{ // not all sends done and timing is OK
		...
		if ((powr.mode > 0) && (powr.nxtTO < (millis() + powr.minTO)))
	...
	if ((send.counter > send.retries) && (send.counter < maxRetries))
	{ // all send but don't wait for an ACK
		send.counter = 0;
		send.timer = 0; // clear send flag
	}

	if ((send.counter > send.retries) && (send.timer <= millis()))
	{ // max retries achieved, but seems to have no answer
		send.counter = 0;
...
{
	if (send.counter > 0)
		return; // send queue is busy, let's wait
	uint8_t len;
...
void HM::send_peer_poll(void)
{
	...
	if (send.counter > 0)
		return; // something is in the send queue, lets wait for the next free slot

	// we are in a loop, therefore check if the request is completed and clear struct
	if (pevt.idx >= peermax[pevt.cnl - 1])
	{ // we are through the list of peers, clear variables
		...
		pevt.idx = 0;
		pevt.sta = 0;
		pevt.act = 0; // clear struct object, no need to jump in again
		return;
	}
	...
	if ((memcmp(peerBuf, broadCast, 4) == 0) || (ret == 0))
	{ // if peer is empty, increase the idx and leave while we are in a loop
		pevt.idx++;
		return;
	}
	...
	if (ret)
	{
		...
		if ((uint16_t)x)
		{																			// if we found the searched string
...
void HM::power_poll(void)
{
	...
	if (powr.mode == 0)
		return; // in mode 0 there is nothing to do
	if (powr.nxtTO > millis())
		return; // no need to do anything
	if (send.counter > 0)
		return; // send queue not empty

	// power mode 2, module is in sleep and next check is reached
	if ((powr.mode == 2) && (powr.state == 0))
	{
		if (cc.detectBurst())
		{										// check for a burst signal, if we have one, we should stay awake
			powr.nxtTO = millis() + powr.minTO; // schedule next timeout with some delay
		}
		else
		{						   // no burst was detected, go to sleep in next cycle
	...
	if ((powr.mode > 1) && (powr.state == 0))
	{			   // TRX module is off, so lets sleep for a while
		...
		if (wd_flag == 1)
		{								 // add the watchdog time to millis()
...
void HM::recv_ConfigPeerAdd(void)
{
	...
	if (recv_ackRq)
		send_ACK(); // send ACK if requested
...
void HM::recv_ConfigWriteIndex(void)
{
	...
	// do something with the information ----------------------------------
	if ((!conf.wrEn) || (!(conf.channel == recv_payLoad[0])))
	{ // but only if we are in config mode
#if defined(AS_DBG)
	...
	if ((recv_ackRq) && (ret == 1))
		...
		send_NACK(); // send NACK while something went wrong
}
...
void HM::recv_Pair_Serial(void)
{
	...
	if (memcmp_P(&recv_payLoad[2], &devParam[3], 10) != 0)
		return;
...
void HM::recv_ConfigStatusReq(void)
{
	...
	if (!ret)
		hm.sendInfoActuatorStatus(recv_payLoad[0], 0xff, 0);
}
/**
...
void HM::recv_PeerEvent(void)
{
	...
	if (!cnl)
		return; // if peer was not found, the function returns a 0 and we can leave
...
#define b_by11 buf[11]
	...
	else if ((b_msgTp == 0x02) && (b_by10 == 0x00))
	{
		if (b_len == 0x0A)
			...
			Serial << F("ACK; data: ") << pHex(&buf[11], b_len - 10);
	}
	...
	else if ((b_msgTp == 0x10) && (b_by10 == 0x01))
	{
		...
		if (b_len >= 19)
			Serial << F(", peer2: ") << pHex(&buf[15], 4);
		if (b_len >= 23)
			Serial << F(", peer3: ") << pHex(&buf[19], 4);
		if (b_len >= 27)
			Serial << F(", peer4: ") << pHex(&buf[23], 4);
	}
...
void HM::initRegisters()
{
	...
	if (tmagicNumber != magicNumber)
	{
		...
		for (uint16_t l = 0; l < sizeof(s_EEPROM); l++)
		{						 // step through the bytes of eeprom
...
uint8_t HM::getSliceDetail(uint8_t cnl, uint8_t lst, uint8_t *peer, s_slcVar *sV)
{
	...
	if (cnl >= devDef.nbrChannels)
		return 0; // channel out of range, end
	if ((peerL == 0) && (lst >= 3))
		return 0; // empty peer is not valid for List3 or List4
	if ((cnl == 0) && (lst != 0))
		return 0; // channel 0 has only a list0

	if (lst < 3)
		peerIdx = 0; // List0 & List1 needs no index
	else
		peerIdx = getIdxByPeer(cnl, peer); // get the peer index of respective peer
	//Serial << "pI:" << peerIdx << ", cnl:" << cnl << '\n';
	if (peerIdx == 0xff)
		return 0; // peer failure

	cType = devDef.chDefType[cnl].type; // is the request for the device or for a channel
	for (cnt = 0; cnt < listTypeDef[cType].nbrLists; cnt++)
	{ // find respective list number
		if (listTypeDef[cType].type[cnt].ListNo == lst)
			break;
		listSliceIdx += listTypeDef[cType].type[cnt].nbrOfSlice; // remember the appropriate listSliceIndex
	}

	if (listTypeDef[cType].nbrLists <= cnt)
		return 0; // list not found
	if (listTypeDef[cType].type[cnt].nbrPeers < peerIdx)
		return 0; // peer out of range
...
uint8_t HM::doesListExist(uint8_t cnl, uint8_t lst)
{
	...
	{ // find respective list number
		if (listTypeDef[cType].type[cnt].ListNo == lst)
			break; // if we have found list4 then break
	}
...
uint8_t HM::getRegList(uint8_t slcPtr, uint8_t slcLen, uint8_t *buf)
{
	...
	for (uint8_t i = slcPtr; i < slcLen; i++)
	{										 // count through slice table
		uint16_t addr = sliceStr[i].regAddr; // position the pointer on the slice string

		for (uint8_t j = 0; j < sliceStr[i].nbrBytes; j++)
		{					   // count through slice
...
void HM::getMainChConfig(void)
{
	...
	{ // count through the channel
		...
		{ // count through the lists, 7 should be the max
			if (!getSliceDetail(i, j, &peer[0], &sV))
				continue; // get the slice details, if empty then next
			if (j == 3)
				l3Ptr[i] = mc[cnt]; // remember list3 pointer
...
void HM::getList3ByPeer(uint8_t cnl, uint8_t *peer)
{
	...
	if (ret)
		getEEpromBlock(sV.phyAddr + (uint16_t)&ee->regs, sV.phyLen, (void *)l3Ptr[cnl]);
...
uint8_t HM::getListForMsg2(uint8_t cnl, uint8_t lst, uint8_t *peer, uint8_t *buf)
{
	...
	if (msgPtr > sV.phyLen)
		return msgPtr = 0; // we are completely through, stop action

	// check to load the storage details only one time
	peerL = *(long *)&peer[0]; // convert byte array to long
	if ((tcnl != cnl) || (tlst != lst) || (tpeerL != peerL))
		msgPtr = 0; // request had changed, start from beginning
	tcnl = cnl;
	tlst = lst;
	tpeerL = peerL; // store for next check

	if (msgPtr == 0)
	{ // only the first time we have to get the details
		if (!getSliceDetail(cnl, lst, peer, &sV))
			return 0xff;
	...
	for (uint8_t i = sV.slcPtr; i < sV.slcLen; i++)
	{ // count through slice table
		if (bInMsg >= bytesPerJunk)
			break;													  // step out if we have enough bytes
		...
		{			  // count through slice
			...
			if (msgCnt > msgPtr)
			{									   // write only to buffer if we need that bytes
	...
	if ((msgPtr == sV.phyLen) && (bInMsg < bytesPerJunk))
	{				  // if we have delivered all bytes and there is some space in the junk
...
//Serial << "we: " << conf.wrEn << ", cnl: " << cnl << ", lst: " << lst << ", peer: " << pHex(peer,4) << '\n';
	...
	if (aLen == 0)
		return 0; // if string is empty we can leave
...
uint8_t HM::getPeerListForMsg(uint8_t cnl, uint8_t *buf)
{
	...
	if (cnl > maxChannel)
		return 0xff; // if channel out of range, return
	...
	if (tPtr > peermax[cnl])
		return tPtr = 0; // tPtr has exceed limit, assumption we are completely through, stop action
	if (cnl != tcnl)
		tPtr = 0; // channel has changed, assume we got a new request
	tcnl = cnl;   // remember channel for next try

	while (tPtr < peermax[cnl])
	{ // step through the peers of specific channel
		if (peerdb[cnl][tPtr])
		{										 // only step in while some content available
			...
			if (cnt >= bytesPerJunk)
				return cnt; // take care of max payload
		}
		else
			tPtr++; // if nothing in, increase the pointer only
	}
...
uint8_t HM::removePeerFromMsg(uint8_t cnl, uint8_t *peer)
{
	...
	if (idx1 != 0xff)
		setEEpromBlock((uint16_t)&ee->peerdb[cnl - 1][idx1], 4, (uint8_t *)&broadCast);

	tPeer[3] = peer[4];				 // change the peer channel
	idx2 = getIdxByPeer(cnl, tPeer); // get the idx of the second peer
	if (idx2 != 0xff)
		setEEpromBlock((uint16_t)&ee->peerdb[cnl - 1][idx2], 4, (uint8_t *)&broadCast);
...
{
	if (memcmp(peer, broadCast, 4) == 0)
		return 0; // return failure while peer is empty

	for (uint8_t i = 0; i < maxChannel; i++)
	{ // step through the channels
		for (uint8_t j = 0; j < peermax[i]; j++)
		{ // step through the peers of channel i
...
uint8_t HM::getIdxByPeer(uint8_t cnl, uint8_t *peer)
{
	...
	if (cnl > maxChannel)
		return 0xff; // check against max channels

	cnl--; // adjust channel due to database start with 0
	for (uint8_t j = 0; j < peermax[cnl]; j++)
	{ // step through the peers of channel i
		if (memcmp((uint8_t *)&peerdb[cnl][j], peer, 4) == 0)
			return j; // return idx if we found something
	}

	if (memcmp(peer, tPeer, 3) == 0)
	{ // dummy peer, FFFFFF01 means index 1
		if (peer[3] >= peermax[cnl])
			return 0xff; // if index out of range, return failure
		else
			return peer[3]; // otherwise give index
	}
...
{
	if (cnl > maxChannel)
		return 0; // check against max channels
	if (idx >= peermax[cnl - 1])
		return 0; // check against max peer slots
...
uint8_t HM::getFreePeerSlot(uint8_t cnl)
{
	cnl--; // adjust channel due to database start with 0
	for (uint8_t j = 0; j < peermax[cnl]; j++)
	{ // step through the peers of channel
		if (memcmp((uint8_t *)&peerdb[cnl][j], broadCast, 4) == 0)
			return j; // return idx if we found a free slot
	}
...
uint8_t HM::countFreePeerSlot(uint8_t cnl)
{
	...
	{ // step through the peers of channel
		if (memcmp((uint8_t *)&peerdb[cnl][j], broadCast, 4) == 0)
			counter++; // increase counter if we found a free slot
	}
...
uint8_t HM::loadDefaultRegset(uint8_t cnl, uint8_t *peer, boolean dual, uint8_t idx)
{
	...
	if (default_regChans_dev[type].regChan_len == 0)
		return 0; // No default. Nothing to do

	// get the slice details and the address list
	s_slcVar sV;
	if (!getSliceDetail(cnl, default_regChans_dev[type].lst, peer, &sV))
		return 0;
...
uint8_t HM::isPeerKnown(uint8_t *peer)
{
	...
	{ // step through the channels
		for (uint8_t j = 0; j < peermax[i]; j++)
		{ // step through the peers of channel i
			if (memcmp((uint8_t *)&peerdb[i][j], peer, 3) == 0)
				return 1; // return 1 if we found something
		}
	}
	return 0; // found nothing, return 0
}
...
{
	if (memcmp(regDev.pairCentral, broadCast, 3) == 0)
		return 1; // return 1 while not paired

	if (memcmp(regDev.pairCentral, pair, 3) == 0)
		return 1; // check against regDev
	else
		return 0; // found nothing, return 0
}
...
//- -----------------------------------------------------------------------------------------------------------------------
void HM::mainSettings(uint16_t *regPtr, uint16_t *peerPtr)
{
	static s_regs reg;
	*regPtr = (uint16_t)&reg;
	*peerPtr = (uint16_t)&peerdb;
	...
	reg.ch_0.pairCentral[0] = HMCID[0];
	reg.ch_0.pairCentral[1] = HMCID[1];
	reg.ch_0.pairCentral[2] = HMCID[2];
...
void BK::poll_btn()
{
	...
	else if ((cStat == 0) && (lStat == 0))
	{
		...
		{			   // long was set last time, should be a double now
...
void RL::trigger11(uint8_t val, uint8_t *rampTime, uint8_t *duraTime)
{
	...
	if (rTime)
		nxtStat = (val == 0) ? 4 : 1; // set next status
	else
		nxtStat = (val == 0) ? 6 : 3;
...
void RL::trigger40(uint8_t lngIn, uint8_t cnt, void *plist3)
{
	...
	if ((lngIn) && (srly->lgMultiExec == 0) && (cnt == rCnt))
		return; // trigger was long
	if ((lngIn == 0) && (cnt == rCnt))
	...
	else if ((actTp == 1) && (lngIn == 1))
	{ // jmpToTarget
		// SwJtOn {no=>0,dlyOn=>1,on=>3,dlyOff=>4,off=>6}
		if (curStat == 6)
			nxtStat = srly->lgSwJtOff; // currently off
		else if (curStat == 3)
			nxtStat = srly->lgSwJtOn; // on
		else if (curStat == 4)
			nxtStat = srly->lgSwJtDlyOff; // delay off
		else if (curStat == 1)
			nxtStat = srly->lgSwJtDlyOn; // delay on
		OnDly = srly->lgOnDly;			 // set timers
		OnTime = srly->lgOnTime;
		OffDly = srly->lgOffDly;
		OffTime = srly->lgOffTime;
	}
	else if ((actTp == 1) && (lngIn == 0))
	{ // jmpToTarget
		if (curStat == 6)
			nxtStat = srly->shSwJtOff; // currently off
		else if (curStat == 3)
			nxtStat = srly->shSwJtOn; // on
		else if (curStat == 4)
			nxtStat = srly->shSwJtDlyOff; // delay off
		else if (curStat == 1)
			nxtStat = srly->shSwJtDlyOn; // delay on
	...
	{									  // toogleToCnt, if tCnt is even, then next state is on
		nxtStat = (cnt % 2 == 0) ? 3 : 6; // even - relay dlyOn, otherwise dlyOff
		OnDly = 0;
		OnTime = 255;
		OffDly = 0;
		OffTime = 255; // set timers
	}
	else if (actTp == 3)
	{									  // toggleToCntInv, if tCnt is even, then next state is off, while inverted
...
{
	if (cbS)
		cbS->sendInfoActuatorStatus(cnlAss, getRly(), getStat()); // call back
}
...
{
	if (prl.nbr == 0)
		return; // no inxtStatnce listed
	for (uint8_t i = 0; i < prl.nbr; i++)
	{							// step through inxtStatnces
...
uint8_t RL::getRly(void)
{
	...
	if ((curStat == 1) || (curStat == 3))
		return 0xC8;
	if ((curStat == 4) || (curStat == 6))
		return 0x00;
	return 0x00; // Default
}
...
{
	if ((rlyTime == 0) || (rlyTime > millis()))
		return;  // timer set to 0 or time for action not reached, leave
	rlyTime = 0; // freeze per default

	// set relay - {no=>0,dlyOn=>1,on=>3,dlyOff=>4,off=>6}
	if (nxtStat == 3 && curStat != 3)
	{ // set relay on
		adjRly(1);
		curStat = 3; // adjust relay, status will send from adjRly()
	}
	else if (nxtStat == 6 && curStat != 6)
	{ // set relay off
		adjRly(0);
		curStat = 6; // adjust relay, status will send from adjRly()
	}
	...
	if (lastTrig == 40)
	{
		...
		else if ((nxtStat == 3) && (OnTime < 255))
		{
			nxtStat = 4;
			if (OnTime)
				rlyTime = millis() + byteTimeCvt(OnTime);
		}
		else if (nxtStat == 4)
		{
			nxtStat = 6;
		...
		else if ((nxtStat == 6) && (OffTime < 255))
		{
			nxtStat = 1;
			if (OffTime)
				rlyTime = millis() + byteTimeCvt(OffTime);
		}
	}
...
{
	if ((cbsTme == 0) || (cbsTme > millis()))
		return; // timer set to 0 or time for action not reached, leave
	if (cbS)
		cbS->sendInfoActuatorStatus(cnlAss, getRly(), 0); // call back
...
void pcInt(uint8_t iPort)
{
	...
	{
		if (tFnd == pci.idx[i])
		{ // found; write flag and time in the respective button key class
